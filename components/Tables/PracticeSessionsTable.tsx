...
import { PracticeSessionData } from "@/Enums/SessionType";
...
const PracticeSessionsTable: React.FC<PracticeSessionsTableProps> = ({ data }) => {
  ...
  const renderActions = (video: PracticeSessionData) => (
    <>
      ...
      <Tooltip title="View" placement="top" arrow>
        <IconButton onClick={() => viewPastSession(video)}>
  ...
  const viewPastSession = (video: PracticeSessionData) => {
    console.log("video", video);
    const videoIdCoded = encodeURIComponent(video.id);
    const titleCoded = encodeURIComponent(video.title);
    const statusCoded = encodeURIComponent(video.status);
    const sessionTypeCoded = encodeURIComponent(video.sessionType);

    router.push({
      pathname: "/past-sessions/view-session",
      query: {
        videoId: videoIdCoded,
        title: titleCoded,
        status: statusCoded,
        sessionType: sessionTypeCoded,
      },
    });
  };
  ...
  function getRandomImageThumbnail(session: PracticeSessionData) {
    if (session.thumbnailUrl === "/react.jpg") {
      return "/react.jpg";
    }
    // Array of image file names
    const images = ["ather.jpg", "izaan.jpg"];

    // Randomly pick an image from the array
    const randomImage = images[Math.floor(Math.random() * images.length)];

    // Return an HTML image tag with the randomly selected image
    return randomImage;
  }
          ...
          <Table stickyHeader aria-label="practice sessions table">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ bgcolor: theme.palette.info.main }}>Title</TableCell>
                <TableCell sx={{ bgcolor: theme.palette.info.main }}>Type</TableCell>
                <TableCell sx={{ bgcolor: theme.palette.info.main }}>Date</TableCell>
                <TableCell sx={{ bgcolor: theme.palette.info.main }}>Time</TableCell>
                ...
                <TableRow hover key={index} sx={{ "& > *": { bgcolor: theme.palette.info.light } }}>
                  <TableCell>
                    <Box sx={{ position: "relative", width: 75, height: 75 }}>
                      <Avatar
                        variant="square"
                        src={getRandomImageThumbnail(session)}
                        alt={session.title} // Ideally, you would use session.name
                  ...
                  <TableCell>{session.creationDate}</TableCell>
                  <TableCell>{session.creationTime}</TableCell>
                  <TableCell align="center">
                    <ProgressContainer>
                      ...
                      {session.status !== "Processing" && (
                        <CircularProgress
                          variant="determinate"
                          value={session.score} // Assuming semi-circle effect is needed, multiply by 2
                          size={65}
                          thickness={4}
                          sx={{
                            borderRadius: "50%",
                            ...
                            [`& .${circularProgressClasses.circle}`]: {
                              strokeLinecap: "round",
                              stroke: getColorForProgressScore(session.score), // Dynamic color for the progress bar
                            },
                          }}
                        />
                      )}
                  ...
                  <TableCell align="center">{renderActions(session)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
