...
function FeedbackStepper() {
  ...
  const scoreLabel = ({ label, value, description }: ScoreLabelProps) => (
          ...
          <Step key={index} expanded>
            ...
            <StepContent>
              ...
              <List dense sx={{ mr: 4 }}>
                {scoreLabel({
                  label: "Correctness",
                  value: item.correctness_score,
                  description: "Measures how accurate the response is to the question asked.",
                })}
                {scoreLabel({
                  label: "Structure",
                  value: item.structure_score,
                  description: "Evaluates the logical organization and clarity of the response.",
                })}
                {scoreLabel({
                  label: "Detail",
                  value: item.detail_score,
                  description:
                    "Assesses the depth of detail and examples provided in the response.",
                })}
                {scoreLabel({
                  label: "Preciseness",
                  value: item.preciseness_score,
                  description:
                    "Rates the specificity and directness of the response to the question.",
                })}
                {scoreLabel({
                  label: "Relevancy",
                  value: item.relevancy_score,
                  description: "Judges how relevant and on-topic the response is.",
                })}
              </List>
