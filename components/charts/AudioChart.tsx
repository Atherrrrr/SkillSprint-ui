...
const AudioChart = () => {
  ...
  const interpolateData = (data: number[], times: number[]): InterpolationResult => {
    const newData: number[] = [];
    const newTimes: number[] = [];
    const steps = 5; // Assuming a fixed number of interpolation steps
  ...
  const options: ApexOptions = {
    chart: {
      height: 350,
      type: "line", // Correctly setting type as a literal 'line'
      zoom: {
        enabled: false,
      },
      toolbar: {
