...
const AppLightTheme: Theme = createTheme({
  palette: {
    mode: "light",
    primary: {
      main: "#00255A",
...
const AppDarkTheme: Theme = createTheme({
  palette: {
    mode: "dark",
    primary: {
      main: "#2A93D5",
...
const MUIThemeProvider: FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentTheme, setCurrentTheme] = useState(responsiveFontSizes(AppDarkTheme));

  useEffect(() => {
    resolvedTheme === "light"
      ? setCurrentTheme(responsiveFontSizes(AppLightTheme))
      : setCurrentTheme(responsiveFontSizes(AppDarkTheme));
  }, [resolvedTheme]);
