...
const DropBox = styled(Box)<DropBoxStyleProps>(
  ({ theme, backgroundColor, borderColor, padding, position }) => ({
    width: "100%",
...
export function DomainImageUpload(props: DomainImageUploadProps) {
  ...
  React.useEffect(() => {
    if (imgBlob && croppedImgUrl && !cropping) {
      // Ensure that the onImageFinal callback is called with both parameters as intended
      ...
      <Grid container gap={2} justifyContent={props.justifyContent || "left"}>
        <Grid item xs={6}>
          ...
          >
            ...
            />
            <label htmlFor="profile-picture">or Choose Image</label>
          </DropBox>
        </Grid>
        <Grid item xs={4}>
          {croppedImgUrl && !cropping ? (
            ...
            >
              <DomainImage alt="not found" src={croppedImgUrl} />
              <IconButton
                onClick={() => handleCropCancel()}
                sx={{
                  position: "absolute",
